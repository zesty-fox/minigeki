...
uint16_t p_b20k_to_joystick(int rawInput) {
  if (rawInput < 0)
    rawInput = 0;
  if (rawInput > 1023)
    rawInput = 1023;

  static const int in[] = {0, 20, 1003, 1023};
  static const int out[] = {0, 0, 1023, 1023};
...
void loop() {
  ...
  for (auto i = 0; i < 8; i++) {
    read += p_b20k_to_joystick(analogRead(LEVER_PIN)) >> 3; // debounce
  }
